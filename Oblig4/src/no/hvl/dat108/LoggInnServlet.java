...
public class LoggInnServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if (username == null || !Validator.isValidUsername(username)) {
        	...
        	finnes = true;
    		Deltager aktuell = deltagerDAO.hentBruker(username);
    		...
    		Passord aktueltPassord = aktuell.getPassord();

    		rettPassord = Passordhjelper.validerMedSalt2(passord, aktueltPassord.getSalt(),aktueltPassord.getPassord());

    		if (finnes && rettPassord)
    			response.sendRedirect(LIST_URL); //rett brukernavn og passord, logges inn
        }
    }
}
